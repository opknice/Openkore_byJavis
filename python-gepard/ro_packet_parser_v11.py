# ro_packet_parser_v11.py
# ================================================================
#  Changes from v10:
#  [FIX]  CRASH: handle_server_broadcast() was called in dispatch()
#         but never defined → NameError killed the session the moment
#         0x0283 arrived. Function is now properly defined.
#  [FIX]  HTTP STREAM POISON: Port 25787 is Gepard's HTTP telemetry
#         (POST /userconfig/load HTTP/1.1). Its MIME boundary bytes
#         (0x2D2D = "--") were flooding enc_buf with ~44–59 junk
#         entries, making ALL key confirmation fail with
#         KEY_PENDING_FAILED enc=0x2D2D→0x???? (unknown).
#         StreamBuf now detects HTTP streams on _sync() and drops
#         them entirely. is_http flag prevents further processing.
#  [FIX]  enc_buf MAP ISOLATION: When the MAP port (24656) first
#         connects, any enc_buf entries accumulated from pre-map
#         phases are cleared. This gives the map-phase key scanner
#         a clean buffer to work with.
#  [FIX]  handle_map_data() had dead broadcast code (copy-paste
#         artifact) that ran after key scan but was never reached
#         via 0x0283 dispatch path — cleaned up.
#  [NEW]  MultiPortManager labels port 22878 as GEPARD_AUTH based
#         on observed session structure.
#  [NEW]  GEPARD_AUTH port (22878) 0x4753 keys are now stored as
#         a separate pending_keys_preauth, NOT overwriting map keys.
#         Only the map-port 0x4753 keys are used for enc decryption.
# ================================================================
...
class StreamBuf:
    ...
    def _process(self):
        if self.is_http:          # [NEW v11] safety guard — should never be reached
            self.buf = b''
            return
...
class MultiPortManager:
    ...
    def get(self, port):
        if port not in self.conns:
            ...
            # [NEW v11] When the MAP port first connects, purge any enc_buf entries
