# ro_packet_parser_v7.py
# ================================================================
#  Changes from v6:
#  [FIX]  import pydivert moved INSIDE main() — log files are now
#         created before WinDivert is touched, so if the script is
#         not run as Administrator or pydivert is missing, the error
#         is written to ro_session_*.log instead of silently vanishing.
#  [NEW]  0x0AC4 (gepard_auth) added to PACKETS table — this packet
#         appears every session right after 0x4753 and contains
#         session-varying fields that may carry Gepard LCG keys.
#         handler calls key_extractor.try_packet() automatically.
#  [NEW]  dispatch(): all unknown server packets ≥32B are now passed
#         to key_extractor.try_packet() — ensures we never miss a
#         key carrier even if its opcode is unrecognised.
# ================================================================
...
def handle_gepard_auth(data, port=0):
    ...
    Scan all offsets for valid LCG key triple."""
    ...
    # Log the changing fields for analysis
    if len(data) >= 16:
        import struct as _s
        fA = _s.unpack_from('<I', data, 4)[0]
        fB = _s.unpack_from('<I', data, 8)[0]
        fC = _s.unpack_from('<I', data, 12)[0]
        LOGK(f"GEPARD_AUTH_FIELDS fA=0x{fA:08X} fB=0x{fB:08X}(acct?) fC=0x{fC:08X}")
    key_extractor.try_packet(data, source=f"0x0AC4:port{port}")
