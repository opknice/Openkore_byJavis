# ro_packet_parser_v6.py
# ================================================================
#  Changes from v5:
#  [ROOT FIX] WinDivert filter now captures ALL ports on SERVER_IP,
#             not just map port 24656 — the Gepard key exchange
#             (0x02AE / 0x083E) happens on the LOGIN or CHAR server
#             connection which we were completely missing before.
#  [FIX]  Remove k3 odd-bit (k3 & 1) filter — too restrictive,
#         valid LCG keys can use even k3 values.
#  [FIX]  StreamBuf._process(): when crypt.on is True, dispatch
#         2-byte client packets through decrypt path instead of
#         pushing to enc_buf.
#  [FIX]  0x00B6 coord decode: try data[2:5] first (correct layout),
#         fall back to data[3:6] if result is out of bounds.
#  [NEW]  MultiPortManager: tracks one StreamBuf pair per port,
#         labels connections as LOGIN / CHAR / MAP.
#  [NEW]  auto-detect server ports from login_success (0x0069) and
#         char_server (0x0071) packets.
#  [NEW]  log_keys.log: dedicated file for every key extraction
#         attempt with full hex context, for offline analysis.
# ================================================================
...
class KeyExtractor:
    ...
    def try_packet(self, data, source=""):
        """Scan data for best key triple. Returns True if keys were set."""
        if crypt.on:
            return True  # already have verified keys

        buf_size = len(self.enc_buf.buf)

        # Log the packet for offline analysis regardless
        ...
        # Scan all 4-byte aligned offsets (and 1-byte steps for thoroughness)
        for offset in range(2, len(data) - 11):
            ...
            # [FIX v6] Only require k2 and k3 non-zero — no odd-bit filter
            if k2 == 0 or k3 == 0:
                continue
        ...
        if best_triple is None and buf_size == 0:
            # No ENC? yet: store speculative keys from offset 4 (primary hypothesis)
            if len(data) >= 16:
                k1, k2, k3 = struct.unpack_from('<III', data, 4)
                if k2 != 0 and k3 != 0:
                    LOGK(f"KEY_SPECULATIVE source={source} off=4 "
                         f"k1={k1:#010x} k2={k2:#010x} k3={k3:#010x}")
                    crypt.set_keys(k1, k2, k3, source=f"{source}[speculative]")
                    return True
            return False
        ...
        if best_triple:
            ...
            # Accept if ratio is good enough, or if buffer is tiny (0-2)
            accept = ratio >= self.MIN_RATIO or checked_n < 3
            if accept:
                crypt.set_keys(k1, k2, k3, source=f"{source}[score={best_score}/{checked_n}]")
...
def main():
    ...
    LOG("  Gepard key exchange 0x02AE/0x083E will be captured from")
