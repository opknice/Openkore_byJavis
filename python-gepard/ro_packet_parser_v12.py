# ro_packet_parser_v12.py
# ================================================================
#  Changes from v11:
#
#  [FIX-CRITICAL] ENC? STREAM STALL / enc_buf CORRUPTION:
#         When crypt.on=False, _process() checked `maybe` (next 2
#         bytes) and if 4 <= maybe <= 32768 it reassigned plen=maybe,
#         causing the parser to wait for e.g. 2993 bytes before
#         continuing. This stalled at position 3 of every initial
#         encrypted burst. Only 3 values reached enc_buf cleanly;
#         the rest were swallowed by fake variable-length packets.
#         Fix: when crypt.on=False, NEVER use the maybe heuristic.
#         Always advance 2 bytes and push to enc_buf. The maybe
#         heuristic is ONLY valid when keys are active and we can
#         decrypt+verify the switch.
#
#  [FIX]  SKIP BYTES LOGGING + KEY SCAN: _sync() now writes all
#         skipped bytes to ro_skip_*.log as hex and calls
#         key_extractor.try_packet() on them. The 167B skipped on
#         the MAP port right after 0x0283 contains unknown Gepard
#         packets whose content is now visible for offline analysis.
#
#  [FIX]  MEMORY ATTACH RETRY: init_memory() is now retried when
#         the MAP port connects, so BamBoo_Client.exe launched after
#         the sniffer starts will still be found.
#
#  [NEW]  MEMORY LCG SCAN: after each ENC? value, scan a configurable
#         memory range for DWORD values whose upper 15 bits match the
#         expected (enc ^ known_opcode) pattern. Finds the runtime
#         LCG cur-state address so nearby DWORDs reveal k2/k3.
#
#  [NEW]  AGGRESSIVE ENC? SCORING: key_extractor now also tries all
#         offsets, not just offset 4, when enc_buf has >= 3 entries.
# ================================================================
...
log_key  = make_logger("keys",    f"{LOG_DIR}/ro_keys_{_ts}.log")
log_skip = make_logger("skip",    f"{LOG_DIR}/ro_skip_{_ts}.log")   # [NEW v12]
...
def LOGK(msg): log_key.info(msg); log_main.info(msg)
def LOGS(msg): log_skip.info(msg); log_main.info(msg)   # [NEW v12] skip bytes
...
PACKETS = {
    ...
    0x01DA: ("login_pin",          -1),
    0x0002: ("bulk_entity_data",  -1),  # [NEW v10] Large map state packet
    ...
    0x0AC4: ("gepard_auth",        -1),  # BamBoo Gepard auth packet
    ...
    0x9103: ("map_data1",          -1),  # [NEW v10] Large map state — scan for keys
    0x2DD7: ("map_data2",          -1),  # [NEW v10] Large inventory/entity dump
    0xC100: ("map_signal_c100",     2),  # [NEW v10] 2-byte map signal
    0xC392: ("gepard_handshake",   44),  # [NEW v10] Gepard challenge (fixed 44B, body=40B)
...
class KeyExtractor:
    ...
    def try_packet(self, data, source=""):
        ...
        [v12] When enc_buf has ≥3 entries, uses _best_candidate() to
        try ALL 4-byte-aligned offsets instead of only offset 4."""
        ...
        if triple is None and buf_size == 0:
            # No ENC? yet — store offset-4 bytes as speculative pending
            if len(data) >= 16:
        ...
        if triple is None and buf_size > 0:
            # [NEW v12] enc_buf has entries but _best_candidate found nothing.
            # Also try ALL 4-aligned offsets in data as fallback.
            for off in range(2, len(data) - 11, 4):
                try:
                    k1, k2, k3 = struct.unpack_from('<III', data, off)
                except struct.error:
                    break
                if k2 == 0 or k3 == 0:
                    continue
                score, checked = self._score(k1, k2, k3)
                if score >= self.MIN_HITS and (checked < 3 or score / checked >= 0.4):
                    LOGK(f"KEY_FALLBACK source={source} off={off} "
                         f"score={score}/{checked} "
                         f"k1={k1:#010x} k2={k2:#010x} k3={k3:#010x}")
                    triple = (k1, k2, k3)
                    break
    ...
    def activate_pending(self, enc_sw):
        ...
        [v12] Also logs the decoded value on failure for offline analysis."""
        ...
        if real_sw in PACKETS:
            ...
            LOGK(f"KEY_PENDING_FAILED source={source} "
                 f"enc=0x{enc_sw:04X} xk=0x{xk:04X} →0x{real_sw:04X} (unknown) "
                 f"k1={k1:#010x} k2={k2:#010x} k3={k3:#010x} — discarding pending")
            self.pending_keys = None
            return False
...
def handle_gepard_handshake(data, port=0):
    """0xC392 — Gepard challenge/handshake packet, fixed 44 bytes.
    Body (bytes 4-43) is 40 bytes of session-specific challenge data.
    The client sends this REPEATEDLY before encryption kicks in.
    We log the body for cross-session analysis and scan for keys."""
    if len(data) < 44: return
    body = data[4:44]
    LOGK(f"GEPARD_HANDSHAKE port={port} body={body.hex()}")
    LOG(f"  [GEPARD] 0xC392 handshake [{len(data)}B] body={body[:8].hex()}...")
    # Scan the body itself for key candidates against enc_buf
    key_extractor.try_packet(data, source=f"0xC392:port{port}")

def handle_map_data(data, sw, port=0):
    """0x9103 / 0x2DD7 — large server packets that may carry Gepard key material."""
    LOG(f"  [MAPDAT] 0x{sw:04X} [{len(data)}B] port={port}")
    if len(data) >= 32 and not crypt.on:
        key_extractor.try_packet(data, source=f"0x{sw:04X}:port{port}")

def handle_server_broadcast(data):
    """0x0283 — server-wide broadcast / notice message.
    [FIX v11] Was called in dispatch() but never defined → NameError crash."""
    if len(data) < 4: return
    body = data[4:]
    try:   text = body.decode('utf-8', errors='replace').rstrip('\x00')
    except: text = body.hex()
    LOG(f"  [BROAD ] {text[:200]}")
    LOGK(f"BROADCAST len={len(data)} text={text[:80]}")
    # Opportunistically scan large broadcasts for key material
    if len(data) >= 32 and not crypt.on:
        key_extractor.try_packet(data, source="0x0283")
...
def dispatch(data, direction, port=0):
    ...
    if sw in (0x02AE, 0x083E):
        ...
        handle_gepard_handshake(d, port)
    elif sw in (0x9103, 0x2DD7):
        handle_map_data(d, sw, port)
    elif sw == 0x0069:
        handle_login_success(d)
    elif sw == 0x0071:
        handle_char_server(d)
    elif sw == 0x4753:
        LOG(f"  [MAP   ] map_login_ack [{len(d)}B] port={port}")
        LOGK(f"MAP_LOGIN_ACK len={len(d)} hex={d[:36].hex(' ')}")
        # [v11] MAP port 0x4753 is the authoritative key source.
        # Use force-scan so it overrides any speculative keys from the pre-map phase.
        if mgr._conn_type(port) == "MAP":
            LOG(f"  [MAP   ] MAP port 0x4753 — force key scan")
            key_extractor.try_packet_force(d, source=f"0x4753:MAP:port{port}")
        elif not crypt.on:
...
class StreamBuf:
    def __init__(self, direction, port=0):
        ...
        self.port   = port
        self.synced = False
        self.is_http = False   # [NEW v11] True once we detect HTTP data

    def feed(self, data):
        if self.is_http:
            return             # [NEW v11] drop all further data from HTTP streams
        self.buf += data
    ...
    def _sync(self):
        ...
        # [NEW v11] HTTP stream detection — Gepard telemetry uses HTTP on side ports.
        # If the first 4 bytes look like an HTTP method or response, mark stream as HTTP
        # and discard immediately to prevent MIME boundary bytes (0x2D2D = "--")
        # from polluting enc_buf.
        if len(self.buf) >= 4:
            hdr = self.buf[:4]
            if hdr in (b'POST', b'GET ', b'HTTP', b'HEAD', b'PUT '):
                self.is_http = True
                LOG(f"  [HTTP  ] p{self.port} {self.dir} HTTP stream detected — suppressing entirely")
                self.buf = b''
                return

        # [FIX v10] Only LOG pre-sync bytes to raw log — do NOT push to
        # enc_buf here.  enc_buf gets populated by _process() naturally
        # when it encounters unrecognised 2-byte switches.  Pushing here
        # AND letting _process() push again caused double-counting.
        if self.dir == "→":
        ...
        for i in range(lim):
            ...
            if sw in PACKETS:
                ...
                if plen >= 4 or (plen == -1 and self._is_good_anchor(sw, i)):
                    if i > 0:
                        skip_bytes = self.buf[:i]
                        LOG(f"  [SYNC  ] p{self.port} {self.dir} skipped {i}B → 0x{sw:04X}")
                        # [NEW v12] Log skipped bytes to dedicated file for offline analysis
                        LOGS(f"SKIP p{self.port} {self.dir} {i}B before 0x{sw:04X} "
                             f"hex={skip_bytes[:64].hex(' ')}")
                        # [NEW v12] Scan skipped bytes for key material
                        if self.dir == "←" and i >= 12 and not crypt.on:
                            key_extractor.try_packet(skip_bytes,
                                                     source=f"skip:p{self.port}:{i}B")
        ...
        for i in range(lim):
            ...
            if sw in PACKETS and PACKETS[sw][1] == 2 and sw != 0x0000:
                if i > 0:
                    skip_bytes = self.buf[:i]
                    LOG(f"  [SYNC  ] p{self.port} {self.dir} skipped {i}B → 0x{sw:04X} (weak)")
                    LOGS(f"SKIP(weak) p{self.port} {self.dir} {i}B before 0x{sw:04X} "
                         f"hex={skip_bytes[:64].hex(' ')}")
                self.buf = self.buf[i:]; self.synced = True; return
        if len(self.buf) > 128:
    ...
    def _process(self):
        if self.is_http:          # [v11] safety guard
            self.buf = b''
            return
        while len(self.buf) >= 2:
            ...
            if plen == -1:
                ...
                if self.dir == "→":
                    # ── CLIENT→SERVER unknown switch ─────────────────────────
                    #
                    # [FIX v12 CRITICAL] When crypt is OFF, we MUST NOT use the
                    # "maybe" variable-length heuristic. The reasoning:
                    #
                    #   Old code checked buf[2:4] and if that looked like a valid
                    #   packet length (4‒32768) it waited for that many bytes before
                    #   advancing. This caused a permanent stall because encrypted
                    #   words (e.g. 0xA8EB → next word 0x0BB1 = 2993 bytes) triggered
                    #   the heuristic and the parser froze at position 3 of every
                    #   initial burst.  Only 3 ENC? values ever reached enc_buf, so
                    #   the key extractor had nothing useful to work with.
                    #
                    #   When crypt is OFF we have no way to distinguish an encrypted
                    #   opcode from the length field of a variable-length packet.
                    #   The safe and correct behaviour is: always advance 2 bytes and
                    #   push to enc_buf — never try to read a "body".
                    #
                    if not crypt.on:
                        # Try to confirm pending speculative keys on first ENC?
                        ...
                        # Keys available: we CAN use the maybe heuristic because we
                        # can decrypt the switch and verify it afterwards.
                        if len(self.buf) < 4: break
...
class MultiPortManager:
    ...
    def _conn_type(self, port):
        ...
        if port == 22878:            return "GEPARD_AUTH"  # [NEW v11] observed pre-map auth port
        return f"PORT{port}"
    ...
    def get(self, port):
        if port not in self.conns:
            ...
            # [NEW v12] If BamBoo_Client wasn't running at startup, retry attach now
            if ctype == "MAP" and _mem_handle is None:
                LOG(f"  [MEM   ] MAP port connected — retrying BamBoo_Client.exe attach")
                init_memory(0)

            # [v11] When the MAP port first connects, purge any enc_buf entries
            # that were accumulated from HTTP Gepard-telemetry side ports. Those MIME
            # boundary bytes (0x2D2D) would cause every pending key confirmation to fail.
            if ctype == "MAP":
                stale = len(enc_buf.buf)
                if stale:
                    enc_buf.buf.clear()
                    LOG(f"  [CONN  ] MAP port {port} — cleared {stale} stale enc_buf entries")
                key_extractor.pending_keys = None   # discard pre-map speculative keys
                LOG(f"  [CONN  ] MAP port {port} — pending_keys reset; ready for clean key scan")
