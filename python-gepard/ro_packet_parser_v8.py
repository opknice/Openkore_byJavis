# ro_packet_parser_v8.py
# ================================================================
#  Changes from v7:
#  [FIX]  handle_server_broadcast() added back — was accidentally
#         omitted in v7 edits, causing crash on every 0x0283 packet.
#  [FIX]  Speculative key logic overhauled: keys from 0x4753 are now
#         stored as "pending" and NOT activated until the first ENC?
#         arrives and the pending triple confirms a known opcode.
#         This prevents wrong speculative keys from blocking 0x0AC4.
#  [FIX]  handle_gepard_auth() now calls try_packet_force() which
#         overrides even active speculative keys — 0x0AC4 is the
#         authoritative key source and must always be scanned.
#  [NEW]  KeyExtractor.activate_pending(): called on first ENC? to
#         confirm or discard pending keys before buffering the value.
# ================================================================
