# ro_packet_parser_v10.py
# ================================================================
#  Changes from v9:
#  [FIX]  StreamBuf._sync(): removed enc_buf.push() — was causing
#         double-counting since _process() would push same values
#         again. Pre-sync bytes are now only LOGGED (raw log) for
#         offline analysis, not fed into enc_buf.
#  [NEW]  0xC392 added to PACKETS as "gepard_handshake" (fixed 44B)
#         — this IS the Gepard challenge packet. Body (40B) is
#         session-specific and logged to ro_keys_*.log.
#  [NEW]  0x9103, 0x2DD7, 0x0002, 0xC100 added to PACKETS
#         — large server packets scanned for LCG key material.
#  [NEW]  handle_gepard_handshake(): extracts and logs the 40-byte
#         Gepard challenge body; calls key_extractor.try_packet().
#  [NEW]  Memory LCG scanner: after connecting, periodically scans
#         a range of memory addresses for values consistent with the
#         observed ENC? LCG sequence (k3 verification).
#  [FIX]  LOG header now says "v10" (was erroneously "v7" in v9).
# ================================================================
